...
短剧 Workbench 会通过所选文本模型生成带有镜头意图和结构化镜头语言的分镜，覆盖景别、运镜、镜头焦段、景深、布光与色温等字段。保存镜头只负责更新元数据并刷新工作流产物；重新生成镜头是独立的视频生成操作，会调用所选视频模型，并把绑定的角色、场景和道具参考一并写入生成提示词。Provider 密钥校验会按所请求的模型做真实验证，Production 面板会实时流式展示任务进度，并在重新打开项目后恢复最近一次渲染报告；一致性检查也会覆盖镜头语言完整性和资产绑定有效性。
