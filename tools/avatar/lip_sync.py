...
class LipSync(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "box": {
                ...
                "maxItems": 4,
                "description": (
                    "Optional fixed face box [top, bottom, left, right]. "
                    "Use when face detection is unavailable or unreliable."
                ),
            },
        },
    }
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        if box is not None:
            if len(box) != 4:
                return ToolResult(
                    success=False,
                    error="box must contain exactly 4 integers: [top, bottom, left, right]",
                )
            cmd.extend(["--box", *[str(v) for v in box]])

        self._prepend_remotion_ffmpeg_to_path()
    ...
    def _prepend_remotion_ffmpeg_to_path(self) -> None:
        """Expose Remotion's bundled ffmpeg/ffprobe to Wav2Lip subprocesses."""
        ffmpeg_dir = (
